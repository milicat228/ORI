    ...
    public partial class Main : Form
    {
        ...
        private void btnLinearRegression_Click(object sender, EventArgs e)
        {
            ...
            foreach(String line in lines)
            {
                ...
                double mort = Double.Parse(parts[2]);
                x.Add(lat);
                y.Add(mort);
            }
        ...
        private void btnKmeans_Click(object sender, EventArgs e)
        {
            ...
            foreach (String line in lines)
            {
                String[] parts = line.Split(',');
                double lat = Double.Parse(parts[1]);
                double l = Double.Parse(parts[4]);               
                Point p = new Point(lat, l);
                kmeansElements.Add(p);
            }
            kmeans.elementi = kmeansElements;
            kmeans.podeliUGrupe(k, toleracijaNaGresku);
