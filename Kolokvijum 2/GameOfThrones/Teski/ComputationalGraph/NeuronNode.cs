...
namespace ComputationalGraph
{
    public class NeuronNode
    {
        ...
        public double forward(List<double> x)
        {
            ...
            //neuronski cvor ima oblik:
            //svaki ulaz se mnozi tezinom, onda se sve to sumira, pa propusti kroz aktivacionu funkciju
            List<double> forSum = new List<double>();            
            for(int i = 0; i < inputs.Count; i++)
            {
                MultiplyNode m = multiplyNodes[i];
                double weigth = m.x[1];
                List<double> newInput = new List<double>();
                newInput.Add(inputs[i]);
                newInput.Add(weigth);
                forSum.Add(m.forward(newInput));
            }
        ...
        public List<double> backward(List<double> dz)
        {
            ...
            backward_signal = activation_node.backward(backward_signal);
            List<double> backward_sum = sumNode.backward(backward_signal);
            for(int i = 0; i < backward_sum.Count; ++i)
            {
                MultiplyNode m = multiplyNodes[i];
                List<double> dx_dw = m.backward(backward_sum[i]);
                dx.Add(dx_dw[0]);
                dw.Add(dx_dw[1]);               
            }
